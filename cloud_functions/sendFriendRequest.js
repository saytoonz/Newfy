...
exports.sendFriendRequest=functions.firestore.document("Notifications/{user_id}/Friend_Requests/{user_email}").onWrite((change,context)=> {
    ...
    return db.collection("Notifications").doc(user_id).collection("Friend_Requests").doc(email_id).get().then((queryResult)=>{
        ...
        return Promise.all([from_data, to_data]).then(result=>{
          ...
          const payload={
                  data:{
                    ...
                    click_action:"com.nsromapa.frenzapp.TARGET_FRIENDREQUEST"
                  }
                };
            
            console.log(" | to: " + from_name+" | from:" + to_name + " | message:" + from_name + " sent you a friend request.");
            ...
            if (error) {
                console.error('Failure sending friend request notification to', tokens[index], error);
